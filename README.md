...
- _Version_: `v1`
- _API point_: `/users`, `/users/<id>`, `/quotes`, `/quotes?authorId=3`, `/authors`, `/infos` 
...
```
    curl -i -H "Accept:application/json" -H "Content-Type:application/json" "http://api.loc/v1/infos"
```
...
"http://api.loc/v1/quotes/1?access_token=76f4c0d40347f24a73799335cefb495be9ea364b"
```

5\. Create User 
```
curl -i -H "Accept:application/json" -H "Content-Type:application/json" -XPOST "http://api.loc/v1/users" -d '{"password": "1234567890", "email": "user@example.com"}'
```
